...
ALLOWED_HOSTS = [
    os.environ.get('ALLOWED_HOST'),         # 'localhost' from env.py (if DEV)
    parsed_gitpod_host,                     # The extracted Gitpod hostname
    'spoodle-space-pp5.herokuapp.com',      # Heroku production frontend app URL
    'spoodlespace.herokuapp.com',           # Heroku production backend app URL
    'localhost',
    '127.0.0.1',
]
...
# --- CORS_ALLOWED_ORIGINS (UPDATED SECTION) ---
# Dynamically pull origins from env.py and include production Heroku origin.
# Add the production React frontend URL from an environment variable
REACT_FRONTEND_PROD_URL = os.environ.get('REACT_FRONTEND_PROD_URL')

CORS_ALLOWED_ORIGINS = [
    'http://localhost:3000',
    os.environ.get('CLIENT_ORIGIN'),      # Will be 'http://localhost:3000' from env.py
    os.environ.get('CLIENT_ORIGIN_DEV'),  # Will be Gitpod 8000- URL from env.py
    "https://3000-samobrienol-spoodlespac-93mpe3hepff.ws-eu120.gitpod.io", # Specific Gitpod frontend preview URL
    'https://spoodle-space-pp5.herokuapp.com', # Explicit frontend Heroku URL
    'https://spoodlespace.herokuapp.com', # Backend's own Heroku URL
]

# Conditionally add the production frontend URL from environment variable
if REACT_FRONTEND_PROD_URL:
    CORS_ALLOWED_ORIGINS.append(REACT_FRONTEND_PROD_URL)
...
# Add backend's own production URL to CSRF_TRUSTED_ORIGINS if needed for self-requests
if os.environ.get('DEV') != 'True':
    CSRF_TRUSTED_ORIGINS.append('https://spoodlespace.herokuapp.com')
if parsed_gitpod_host:
    # Adding the parsed Gitpod hostname with potential 8000 port for CSRF if needed by the frontend
    # For Gitpod, the client typically connects to port 3000, so this might be redundant with the 3000- URL
    # but including it for thoroughness if the 8000- URL is directly accessed.
    CSRF_TRUSTED_ORIGINS.append(f"https://{parsed_gitpod_host}")
...
# This line is crucial for collectstatic to find your custom static/html/cloudinary_cors.html
STATICFILES_DIRS = [
